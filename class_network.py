...
def model():
    ...
    model.add(Dense(10, activation="softmax")) # 10 output nodes, one for each class
...
with open("output_class.txt", "a") as output:
    output.write("CLASS accuracy: %.2f%% (%.2f%%)\n" % (np.mean(accuracy), np.mean(std_dev)))
    output.write("CLASS misclassification rate: %.2f%% (%.2f%%)\n" % (np.mean(misclassification), np.std(misclassification)))
    output.write("CLASS mean squared error: %.2f (%.2f%%)\n" % (np.mean(mse), np.std(mse)))
    output.write("CLASS recall: %.2f%% (%.2f%%)\n" % (np.mean(recall), np.std(recall)))
    output.write("CLASS precision: %.2f%% (%.2f%%)\n" % (np.mean(precision), np.std(precision)))